...
/// SPARSE_MAX elements. Used by HybridBitSet; do not use directly.
///
...
/// than SPARSE_MAX. It's especially efficient for sets that typically have a
/// small number of elements, but a large `domain_size`, and are cleared
/// frequently.
#[derive(Clone, Debug)]
...
fn matrix_intersection() {
    ...
    let intersection = matrix.intersection(2, 64);
    assert!(intersection.is_empty());

    let intersection = matrix.intersection(2, 65);
